    ...
    <el-dialog v-model="EditAdminFormVisible" title="Edit Admin User" draggable>
      <el-form :model="editAdminData">
        ...
        <el-form-item label="Permission" >
          <el-select class="el-select" v-model="editAdminData.permission_str" placeholder="Select" size="large" @change="setPermission">
...
const AddAdmin = async (action, visable) => {
  ...
  if (action === 'confirm') {
    let sendData = {  first_name: AddAdminData.first_name, last_name: AddAdminData.last_name,
...
const MongoQurey = async (queryData) => {
  ...
  for (let i = 0; i < UserData.length; i++) {
    console.log(UserData[i].permission.user)
