<template>
    <div class="parking" style="height:85vh;">
      ...
      <!-- <el-checkbox v-model="not_end_time_visible" label="not exist End Time" size="large" /> -->
      <el-table :data="parkingData" style="width: 95%; height:95%" stripe :cell-style=msi.tb_cell :header-cell-style=msi.tb_header_cell size="large">
        <!-- <el-table-column prop="locationPk" label="locationPk" min-width="80"/> -->
        ...
        <el-table-column prop="car_lp_pic" label="Pic" min-width="100">
          <template #default="scope">
            <template  v-if="!scope.row.show">
              <el-button @click="downloadImage(scope.row)">
                preview
              </el-button>
            </template>
            <template v-else>
            ...
            <a :href="scope.row.car_lp_pic" download="image.jpg"> </a>
...
const show_button = ref(true)



const all_visible = ref(false)
const not_end_time_visible = ref(true)
const downloadImage = (ttt) => {
  ttt.show = true
}
