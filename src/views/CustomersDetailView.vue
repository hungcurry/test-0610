		...
		<div class="customers-el-tab">
			<el-tabs v-model="activeName" class="demo-tabs" >
                ...
                <div class="real-time-info-item">
                  ...
                  <el-button  v-if="company === 'MSI'" class="gear" @click="clearEvseList"><font-awesome-icon icon="fa-solid fa-gear" /> Clear</el-button>
								</div>
                <div class="real-time-info-item">
                ...
                <div class="real-time-info-item">
									...
									<p class="real-time-info-value">{{ paymentData.charge_hr + ":" + paymentData.charge_min + ":" + paymentData.charge_sec}}</p>
								</div>
							</div>
						</div>
					</div>
					...
					<div class="customers-down">
						<div class="rfid">
							<div class="customers-general-info-header">
									<font-awesome-icon icon="fa-regular fa-user" />
									RFID
									<el-button class="gear" @click="editRfid"><font-awesome-icon icon="fa-solid fa-gear" /> Add RFID</el-button>
							</div>
              <div class="rfid-detail">
                <el-table :data="userData.rfids" style="width: 95%; height:95%" stripe :cell-style=msi.tb_cell :header-cell-style=msi.tb_header_cell size="large">
                  ...
                  <template #default="scope" >
                    <el-button @click="card_detail(scope)"> <font-awesome-icon icon="fa-solid fa-ellipsis" /> </el-button>
                  </template>
                  </el-table-column>
                </el-table>
							</div>
				...
				<el-tab-pane label="Payment" name="second">
					<el-table :data="paymentData" style="width: 95%; height:900px" stripe :cell-style=msi.tb_cell  :header-cell-style=msi.tb_header_cell size="large" v-loading = "isLoading">
...
const confirmRfid = async (confirm) => {
	...
	if (confirm === 'confirm') {
		if (modify_card_index.value === -1) {
			userData.rfids.push({rfid : rfidData.rfid.toUpperCase(), cash : parseInt(rfidData.cash),
...
onMounted( async() => {
  ...
  for (let i = 0 ; i < userData?.paylist?.length; i++) {
    console.log(userData.paylist.length)
